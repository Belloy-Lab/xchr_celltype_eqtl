# XWAS summary stats -> coloc .ma files (one per trait x model) with case proportion s, variant IDs aligned to the chrX bfile
rm(list = ls())
library(dplyr)
library(data.table)
# CONFIG
...
prepare_coloc_sumstats <- function(df_GWAS, bfile_id,
                                   EA_col, NEA_col, BP_col,
                                   BETA_col, SE_col, P_col, N_col, FREQ_col, s_col,
                                   chr_prefix = "chrX") {
  df <- copy(df_GWAS)
...
# 3. Job table; n_manual = N when the file has no N_incl column; s = s_cases / s_total (NA = file has s); flip_freq: freq -> 1 - freq
jobs <- data.frame(
  ...
  n_manual = c(NA, NA, NA, NA,
               ...
               NA, NA, NA, NA,
               6614, NA, 3606, 3008),
  s_cases = c(NA, NA, NA, NA,
              888, 888, 472, 416,
              16521, 16521, 10002, 5328,
              2591, 2591, 1643, 948),
  s_total = c(NA, NA, NA, NA,
              8016, 8016, 3859, 4157,
              564139, 564139, 261231, 302908,
              6614, 6614, 3606, 3008),
  flip_freq = c(FALSE, FALSE, FALSE, FALSE,
...
for (i in seq_len(nrow(jobs))) {
  ...
  if (!is.na(job$s_cases))  df_GWAS$s      <- job$s_cases / job$s_total
  
  sc <- schemas[[job$schema]]
  df_COLOC <- prepare_coloc_sumstats(
    df_GWAS  = df_GWAS,
    bfile_id = bfile_id,
    EA_col   = sc$EA,   NEA_col = sc$NEA, BP_col   = sc$BP,
    BETA_col = sc$BETA, SE_col  = sc$SE,  P_col    = sc$P,
    N_col    = sc$N,    FREQ_col = sc$FREQ, s_col  = "s"
  )
  
  if (job$flip_freq) df_COLOC$freq <- 1 - df_COLOC$freq
  
  fwrite(df_COLOC, paste0(job$name, ".ma"), sep = "\t", quote = FALSE)
  cat(sprintf("[%d/%d] %s -> %s.ma  (%d variants)\n",
              i, nrow(jobs), job$name, job$name, nrow(df_COLOC)))
}