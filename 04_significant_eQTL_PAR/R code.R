# Significant PAR cis-eQTLs: two-step FDR, then LD clumping (r2 < 0.1) for independent top signals
rm(list = ls())
library(data.table)
library(dplyr)
library(ieugwasr)
...
bfile_prefix      <- "../00.2_PAR_chr_bfile_QC/chrX_QC_EU.0.G"
...
for (i in 1:nrow(cell_type_setting_combination)) {
  ...
  save_name <- paste0("eqtl_res_PAR_chr_",ct,"_",st,".rds")
...
writexl::write_xlsx(cell_type_setting_combination,"N_of_cis_eQTL_PAR.xlsx")
df_eqtl_cis_gene_level <- rbindlist(
  df_eqtl_cis_sort_list,
  use.names = TRUE,
  fill = TRUE
)
writexl::write_xlsx(df_eqtl_cis_gene_level,"Details_of_cis_eQTL_PAR.xlsx")
# 3. Patch ieugwasr::ld_clump_local to pass --allow-extra-chr (PAR1/PAR2)
...
for (i in 1:length(setting_all)) {
  ...
  for (j in 1:nrow(analysis_list)) {
    ...
    read_name <- paste0("eqtl_res_PAR_chr_",analysis_list$cell_type[j],"_",analysis_list$setting[j],".rds")
  ...
  saveRDS(df_eqtl_sig_all, paste0("eqtl_sig_all_PAR_chr_2stepFDR_before_prune_",setting_all[i],".rds"))
  saveRDS(df_eqtl_sig_clumped_all, paste0("eqtl_sig_clumped_all_PAR_chr_2stepFDR_after_prune_",setting_all[i],".rds"))
  writexl::write_xlsx(df_eqtl_sig_clumped_all, paste0("eqtl_sig_clumped_all_PAR_chr_2stepFDR_after_prune_",setting_all[i],".xlsx"))
}