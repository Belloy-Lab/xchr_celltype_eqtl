# Basic PAR variant/sample QC: geno/mac, differential missingness, HWE, split PAR, keep PAR1/PAR2, same samples as non-PAR QC
rm(list = ls())
library(data.table)
library(plinkbinr)
...
out_file     <- "chrX_QC"
nonpar_fam   <- "../00.1_X_chr_bfile_QC/chrX_QC_EU.0.E.2.fam"   # keep the same samples as the non-PAR QC
...
command <- paste("plink2",
                 ...
                 sep = " ")
system(command)
# 7. Keep non-PAR QC samples; control allele frequencies
command <- paste("plink2",
